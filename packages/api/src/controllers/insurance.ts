...
export async function getWorkerInsurance(req: Request, res: Response) {
  try {
    const docs = await insuranceService.getWorkerInsurance(requireParam(req, 'id'))
...
export async function updateInsuranceStatus(req: Request, res: Response) {
  try {
    ...
    const doc = await insuranceService.updateInsuranceStatus(requireParam(req, 'docId'), status)
